...
const SessionPage: React.FC = () => {
  ...
  // Initialize all Audio files on Mount
  useEffect(() => {
    ...
    bgAudio.current = setupAudio('/bg.mp3', true, 0.2); // Loop at 30% volume

    return () => {
      // Cleanup: Stop background music if user leaves page
  ...
  const playSound = async (type: Phase | 'BG') => {
    if (type === 'BG') {
      ...
      } catch (e) { console.warn("BG Audio blocked", e); }
      return;
    }

    // Stop cues to prevent overlapping
    ...
    if (target) {
      try {
        await target.play();
      } catch (e) { console.warn("Cue Audio blocked", e); }
    }
  };
  ...
  useEffect(() => {
    ...
    if (isActive) {
      // Start Background music and first cue
      playSound('BG');
      ...
      // Pause background music if user pauses the session
      bgAudio.current?.pause();
    }
  ...
  }, [isActive, phase, inhale, hold, exhale, totalSeconds, secondsInPhase]);
  ...
  return (
    <div className="flex flex-col items-center justify-center min-h-screen bg-slate-950 text-white p-6 font-sans overflow-hidden">
      ...
      <div className={`fixed inset-0 opacity-10 transition-colors duration-1000 blur-[100px] -z-10 ${
        phase === 'INHALE' ? 'bg-blue-500' : phase === 'HOLD' ? 'bg-purple-500' : 'bg-emerald-500'
      }`} />
      ...
      <div className="relative flex items-center justify-center w-80 h-80">
        <AnimatePresence mode="wait">
          <motion.div
            key={phase}
            initial={{ scale: phase === 'INHALE' ? 0.7 : 1.4 }}
            animate={{ scale: phase === 'EXHALE' ? 0.7 : 1.4 }}
            transition={{ 
              duration: phase === 'HOLD' ? 0 : (phase === 'INHALE' ? inhale : exhale), 
              ease: "linear" 
            }}
            className={`absolute w-full h-full border-[1px] rounded-full opacity-40 ${
              phase === 'INHALE' ? 'border-blue-400' : phase === 'HOLD' ? 'border-purple-400' : 'border-emerald-400'
            }`}
          />
        </AnimatePresence>
        ...
        <div className="text-center z-10">
          <motion.h2 
            ...
            className="text-8xl font-thin font-mono mb-2"
          >
            {secondsInPhase}
          </motion.h2>
          <p className="text-xs tracking-[0.5em] font-light uppercase text-slate-400">
            {phase}
          </p>
        </div>
      </div>
      ...
      <div className="mt-20 flex justify-between w-full max-w-xs border-t border-white/5 pt-10">
        ...
        <div className="text-center">
          ...
          <p className="text-xl font-mono text-slate-300">
            {Math.floor(totalSeconds / (inhale + hold + exhale))}
          </p>
        </div>
      </div>
      ...
      <button
        ...
        className={`mt-14 w-full max-w-xs py-5 rounded-2xl font-bold tracking-[0.2em] transition-all duration-500 uppercase text-xs border ${
          isActive 
            ? 'bg-transparent text-red-500 border-red-900/50 hover:bg-red-500/5' 
